...
Bsec bme688; // BME680 sensor, used for gas resistance and temperature/humidity/pressure
Sensor sensor(scd4x, sen5x, bm8563, bme688);
...
bool checkBme688SensorStatus(void);
...
//    pinMode(GROVE_SCL, OUTPUT);
    ...
    {
        log_i("BME688 sensor initialized successfully. Sending Subscribe.");
...
bool checkBme688SensorStatus(void)
{
    if (bme688.bsecStatus != BSEC_OK) {
        if (bme688.bsecStatus < BSEC_OK) {
            log_w("BSEC error code : %s", String(bme688.bsecStatus));
        } else {
            log_w("BSEC warning code : %s", String(bme688.bsecStatus));
        }

        return false;
    }

    if (bme688.bme68xStatus != BME68X_OK) {
        if (bme688.bme68xStatus < BME68X_OK) {
            log_w("BME68X error code : %s", String(bme688.bme68xStatus));
        } else {
            log_w("BME68X warning code : %s", String(bme688.bme68xStatus));
        }
        return false;
    }
